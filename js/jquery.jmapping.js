    ...
    if (!($(map_elm).data('jMapping'))){ // TODO: Should we use a different test here?
      ...
      var attachMapsEventToLinks = function(){
        ...
        $(location_link_selector).on('click', function(e){
          ...
          //Start Patch - Kevin Williams --> 
          //On click center map and show info-box
          var lat, lng, currentZoom, zoomIn, clickZoom;
          //get lat and lng from link
          lat = ($(this).attr('latitude'));
          lng = ($(this).attr('longitude'));

          //center map to clicked link location
          map.setCenter(new google.maps.LatLng(lat, lng));
          //get the current zoom
          currentZoom = map.getZoom();
          //get default zoom from link
          zoomIn = parseInt(($(this).attr('zoom')));
          //if default zoom level is less then use the current zoom level
          clickZoom = (zoomIn < currentZoom) ? currentZoom : zoomIn;
          //set zoom on map
